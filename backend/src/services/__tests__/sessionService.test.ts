...
describe('web session revocation', () => {
  ...
  afterAll(async () => {
    await pool.query(
      ...
      [sessionId, otherSessionId, expiredSessionId]
    );
    await pool.end();
  });
  ...
  it('accepts a matching, unexpired session ID', async () => {
    ...
    await expect(hasMatchingRefreshSession(accountName, sessionId, refreshToken)).resolves.toBe(true);
  });
  ...
  it('rejects missing, mismatched, or cross-session handles', async () => {
    ...
    await expect(hasMatchingRefreshSession(accountName, sessionId, otherRefreshToken)).resolves.toBe(false);
    await expect(hasMatchingRefreshSession(accountName, otherSessionId, refreshToken)).resolves.toBe(false);
  });
  ...
  it('rejects a session after its database row is revoked', async () => {
    ...
    await expect(hasMatchingRefreshSession(accountName, sessionId, refreshToken)).resolves.toBe(false);
  });
  ...
  it('rejects an expired session even when its ID and token match', async () => {
    ...
    await expect(hasMatchingRefreshSession(accountName, expiredSessionId, expiredRefreshToken)).resolves.toBe(false);
  });
});
