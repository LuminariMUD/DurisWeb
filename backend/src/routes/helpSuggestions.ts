...
router.delete(
  '/guide/suggestions/:id',
  requireAuth,
  suggestionMutationLimiter,
  async (req, res) => {
    try {
      const id = parseHelpSuggestionId(req.params.id);
      const accountName = req.user!.accountName;
...
router.get(
  ...
  requireAuth,
  requirePermission('manage_help_suggestions'),
  async (req, res) => {
    try {
...
router.get(
  '/admin/help-suggestions/pending-count',
  requireAuth,
  requirePermission('manage_help_suggestions'),
  async (_req, res) => {
    try {
...
router.patch(
  '/admin/help-suggestions/:id/review',
  requireAuth,
  requirePermission('manage_help_suggestions'),
  suggestionMutationLimiter,
  async (req, res) => {
    try {
      ...
      const reviewed = await helpSuggestionService.reviewSuggestion(
        id,
        action,
        accountName,
        reviewerNotes,
