...
const emit = defineEmits<{
  ...
  (e: 'positions-changed', positions: Record<number, RoomPosition>): void
}>()
...
function buildElements(): ElementDefinition[] {
  ...
  const savedPos = props.savedPositions || {}

  // Add nodes
  for (const room of props.rooms) {
    const colors = getSectorColor(room.sectorType)
    const pos = savedPos[room.vnum]
    ...
    elements.push({
      ...
      data: {
        ...
        hasUp: Boolean(room.exits?.up),
        hasDown: Boolean(room.exits?.down),
        bgColor: colors.bg,
...
function initCytoscape() {
  ...
  if (props.rooms.length === 0) return // Don't init if no rooms
  ...
  const hasPositions = props.savedPositions && Object.keys(props.savedPositions).length > 0
  ...
  try {
    cyRef.value = cytoscape({
      ...
      elements,
      style: stylesheet,
      layout: hasPositions ? { name: 'preset' } : { name: 'grid', rows: 1 },
      minZoom: 0.25,
      maxZoom: 2,
    })
  } catch (error) {
  ...
  // Event: node drag end - snap to grid
  ...
  cy.on('zoom', () => {
    ...
    tooltipVisible.value = false // Hide tooltip while zooming
  })

  // Event: pan
  cy.on('pan', () => {
    tooltipVisible.value = false // Hide tooltip while panning
  })

  // Run auto-layout if no saved positions
  if (!hasPositions && props.rooms.length > 0) {
    nextTick(() => {
      runAutoLayout()
    })
  }
...
function emitPositions() {
  ...
  const positions: Record<number, RoomPosition> = {}
  cy.nodes().forEach((node: NodeSingular) => {
    const vnum = node.data('vnum') as number
    const pos = node.position()
    positions[vnum] = { x: pos.x, y: pos.y }
  })
...
function toggleFullscreen() {
  ...
  // After DOM update, resize cytoscape
  nextTick(() => {
    const cy = cyRef.value
    if (cy) {
      cy.resize()
...
async function saveAsPng() {
  ...
  try {
    // Get the map PNG from cytoscape
    const mapPng = cy.png({
      output: 'blob',
    ...
    ctx.fillText(`${props.rooms.length} rooms`, padding, padding + 32)

    // Draw map image
    ctx.drawImage(mapImage, padding, headerHeight + padding)

    // Draw legend box
    const legendX = mapImage.width + padding * 2
    const legendY = headerHeight + padding
    ...
    ctx.fillText('Sector Types', legendX + legendPadding, legendY + legendPadding)

    // Draw legend items
    ctx.font = '12px system-ui, -apple-system, sans-serif'
    let itemY = legendY + legendPadding + 24

    for (const [sectorId, name] of Object.entries(SECTOR_NAMES)) {
      const colors = SECTOR_COLORS[Number(sectorId)] || DEFAULT_SECTOR_COLOR

      // Draw color box
      ctx.fillStyle = colors.bg
      ctx.fillRect(legendX + legendPadding, itemY, 16, 16)
      ctx.strokeStyle = colors.border
      ctx.lineWidth = 2
      ctx.strokeRect(legendX + legendPadding, itemY, 16, 16)

      // Draw name
      ctx.fillStyle = '#d4d4d8' // zinc-300
      ctx.fillText(name, legendX + legendPadding + 24, itemY + 12)
  ...
  >
    ...
    <div class="px-3 py-2 border-b border-zinc-800 flex items-center justify-between shrink-0">
      <div v-if="isFullscreen">
        <h2 class="text-lg font-semibold text-zinc-100" v-html="zoneNameAnsi ? parseAnsiToHtml(zoneNameAnsi) : (zoneName || 'Zone Map')" />
      ...
      <div class="flex items-center gap-2">
        ...
        <Button
          ...
          title="Save as PNG"
          @click="saveAsPng"
          :disabled="isSavingPng || rooms.length === 0"
        >
          <Download :class="isFullscreen ? 'h-4 w-4' : 'h-3.5 w-3.5 text-zinc-500'" />
        </Button>
        <Button
          variant="ghost"
          size="icon"
...
<style>
/* Cytoscape container needs explicit dimensions */
.cy-container {
  width: 100%;
  height: 100%;
}
