...
const forumPostCallbacks = ref<
  Array<(data: { threadId: number; post: any; authorAccount: string }) => void>
>([])
...
const deleteProgressCallbacks = ref<
  Array<(data: { requestId: string; message: string; status: string }) => void>
>([])
...
export function useWebSocket() {
  ...
  const onForumPost = (
    callback: (data: { threadId: number; post: any; authorAccount: string }) => void,
  ) => {
  ...
  const onDeleteProgress = (
    callback: (data: { requestId: string; message: string; status: string }) => void,
  ) => {
    deleteProgressCallbacks.value.push(callback)
  }

  const offDeleteProgress = (
    callback: (data: { requestId: string; message: string; status: string }) => void,
  ) => {
