...
async function loadfaq() {
  loading.value = true
  try {
    const response = await api.get('/api/content/faq')
    // Convert ANSI codes to HTML for TipTap editor
    const ansiText = response.data.faq || ''
    faqContent.value = ansiToHtmlWithStyles(ansiText)
  } catch (error: any) {
    toast.show({
      title: 'Error',
      message: error.response?.data?.error?.message || error.message || 'Failed to load faq',
      type: 'error',
    })
  } finally {
    loading.value = false
...
async function savefaq() {
  if (!faqContent.value.trim()) {
    toast.show({
      ...
      message: 'faq content cannot be empty',
      type: 'error',
    })
    return
  }
  ...
  try {
    ...
    const ansiText = htmlToAnsi(faqContent.value)
    await api.put('/api/content/faq', {
      content: ansiText,
    })
    ...
    toast.show({
      title: 'Error',
      message: error.response?.data?.error?.message || error.message || 'Failed to save faq',
      type: 'error',
    })
  } finally {
    saving.value = false
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-6">
      ...
      <Button @click="savefaq" :disabled="saving || loading">
