...
async function loadrules() {
  loading.value = true
  try {
    const response = await api.get('/api/content/rules')
    // Convert ANSI codes to HTML for TipTap editor
    const ansiText = response.data.rules || ''
    rulesContent.value = ansiToHtmlWithStyles(ansiText)
  } catch (error: any) {
    toast.show({
      title: 'Error',
      message: error.response?.data?.error?.message || error.message || 'Failed to load rules',
      type: 'error',
    })
  } finally {
    loading.value = false
...
async function saverules() {
  ...
  try {
    ...
    const ansiText = htmlToAnsi(rulesContent.value)
    await api.put('/api/content/rules', {
      content: ansiText,
    })
    ...
    toast.show({
      title: 'Error',
      message: error.response?.data?.error?.message || error.message || 'Failed to save rules',
      type: 'error',
    })
  } finally {
    saving.value = false
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="text-3xl font-bold text-white">rules Management</h1>
        <p class="text-muted-foreground mt-1">
          Manage the MUD Rules shown to players on login
        </p>
        <p v-if="lastUpdate" class="text-xs text-muted-foreground mt-1">
          Last updated: {{ formatDate(lastUpdate) }}
        </p>
      </div>
      <Button @click="saverules" :disabled="saving || loading">
    ...
    <div class="mt-6 border border-blue-500/30 bg-blue-500/10 rounded-lg p-4">
      <h3 class="font-medium text-blue-400 mb-2">About rules</h3>
      <p class="text-sm text-gray-300">
        The MUD Rules (rules) is displayed to all players detailing allowed and prohibited behaviors.
