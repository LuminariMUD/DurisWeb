...
| DB-04 | Done. Request handlers no longer leak a relaxed session SQL mode to the pool; the value is captured, restored, and the connection discarded if restoration fails. Sampled checkout telemetry re-verifies the session invariants every five minutes and alerts on drift without a restart. | — |
...
| DB-10 | Partial. Preflight gained the SQL-artifact gate and the auction engine/column contract; MUD-write allowlist verified and enforced in CI. The verifier now fails when the copied table baseline diverges from the MUD repository's manifest (DurisMUD is checked out in CI), classifies multi-table `DELETE`/`UPDATE` forms, and rejects dynamic table targets, and `import-wiki-data.ts` publishes only literal SQL. | Broader feature-contract verification remains open. The two allowlist entries awaiting MUD-owner confirmation (`timers`, `zones` resets) stay noted until the MUD owner confirms those contracts. |
